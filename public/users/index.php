...
if (!isset($_SESSION["user"]) || !UserValidator::hasValidRole($_SESSION["user"]["role"], Role::Admin)) Navigation::redirect("../cars/index.php");
                ...
                <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
                    ...
                    <div class="hidden sm:ml-6 sm:block">
                        <div class="flex space-x-4">
                            ...
                            <a href="../cars/index.php" class="rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white"
                                aria-current="page">Cars</a>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <?php foreach (UserModel::fetchAllUsers() as $user):
                                $roleColor = (UserValidator::hasValidRole($user["role"], Role::Admin)) ? "red" : "blue";
                            ?>
                                    ...
                                    <th scope="row"
                                        ...
                                        <div class="shrink-0 flex items-center">
                                            <img class="h-12 w-12 object-cover rounded-full mr-3"
                                                src="<?= $user["image"] ?>" alt="Current profile photo" />
                                            <?= $user["username"] ?>
                                        </div>
                                    </th>
                                    <td class="px-4 py-3">
                                        <span
                                            class="bg-primary-100 text-primary-800 text-xs font-medium px-2 py-0.5 rounded dark:bg-primary-900 dark:text-primary-300"><?= $user["email"] ?></span>
                                    </td>
                                    <td class="px-4 py-3">
                                        <span
                                            class="bg-<?= $roleColor ?>-100 text-<?= $roleColor ?>-900 text-xs font-medium text-bold px-2 py-0.5 rounded dark:bg-<?= $roleColor ?>-900 dark:text-<?= $roleColor ?>-300"><?= $user["role"] ?></span>
                                    </td>
                                    <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        <form action="delete.php" method="POST">
                                            <div class="flex items-center space-x-4">
                                                <input type="hidden" name="id" value="<?= $user["id"] ?>" />
                                                <a href="update.php?id=<?= $user["id"] ?>" data-drawer-target="drawer-update-product"
