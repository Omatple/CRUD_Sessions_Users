...
if (
    ...
    !$id || (!$car = CarModel::getCarByField('id', $id))
) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['resetDefaultImage'])) {
        CarModel::resetCarImageToDefault($id);
        CarValidator::deleteOldImage($car['image']);
        $car['image'] = "img/" . ImageConstants::DEFAULT_FILENAME;
        ...
        if (!$hasErrors && !CarValidator::isUniqueCar($name, $brand, $horsepower, $id)) {
            $hasErrors = true;
        }
        ...
        if (basename($car['image']) !== basename($image)) {
            CarValidator::deleteOldImage($car['image']);
        }
...
<body class="bg-gray-50 dark:bg-gray-900 antialiased">
    <section class="bg-white dark:bg-gray-900">
        <div class="px-4 mx-auto max-w-2xl lg:py-16">
            <h2 class="mb-4 text-2xl font-bold text-gray-900 dark:text-white">Update Car</h2>
                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    ...
                    <div>
                        <div class="flex items-center space-x-6 mt-4">
                            <div class="shrink-0">
                                <img id='preview_img' class="h-16 w-16 object-cover rounded-full" src="<?= $car["image"] ?>"
                    ...
                    <div>
                        ...
                        <input type="number" name="horsepower" id="horsepower" value="<?= $car["horsepower"] ?>"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
